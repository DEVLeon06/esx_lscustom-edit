...
AddEventHandler('esx_lscustom:cancelInstallMod', function()
    if originalPrices then
        for k, v in pairs(Config.Menus) do
            if originalPrices[k] then
                if type(originalPrices[k].price) == "table" then
                    for i = 1, #originalPrices[k].price do
                        v.price[i] = originalPrices[k].price[i]
                    end
                elseif type(originalPrices[k].price) == "number" then
                    v.price = originalPrices[k].price
                end
            end
        end
        Config.DefaultWheelsPriceMultiplier = originalPrices["DefaultWheelsPriceMultiplier"] or 1.0
        originalPrices = nil
    end
...
RegisterCommand("tuning", function()
    ESX.TriggerServerCallback("esx_lscustom:isAdmin", function(isAdmin)
        if isAdmin then
            local playerPed = PlayerPedId()
            if IsPedInAnyVehicle(playerPed, false) then
                local vehicle = GetVehiclePedIsIn(playerPed, false)
                ...
                for k, v in pairs(Config.Menus) do
                    ...
                    if type(v.price) == "table" then
                        originalPrices[k].price = {}
                        for i = 1, #v.price do
                            originalPrices[k].price[i] = v.price[i]
                            v.price[i] = 0
                        end
                    elseif type(v.price) == "number" then
                        originalPrices[k].price = v.price
                        v.price = 0
                    end
                end

                originalPrices["DefaultWheelsPriceMultiplier"] = Config.DefaultWheelsPriceMultiplier
                Config.DefaultWheelsPriceMultiplier = 0
